...
5. For Owner-authored or company-shared skills, use owner.write_skill, assign the skill to the exact user/Agent/group with owner.assign_resource, then verify the target Agent runtime map and native settings before claiming the skill is available there.

## Success evidence
- skill package validates.
- local skill ref or promoted Hub skill ref is returned.
- target Agent map/native settings show the assigned skill when the skill is meant for another Agent.
