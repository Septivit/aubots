...
3. Call owner.prepare_personal_agent with display_name, telegram_username, telegram_user_id when known, language/timezone override when needed, purpose, model, effort, interface_kind, and initial_context. Treat @username as a label; do not claim Telegram routing is live until the numeric id is known or the user sends the bot a first message.
...
11. End with an Owner-facing people-and-access snapshot: who was invited/prepared, which Personal Agent or Work Agent they can use, what resources/skills/connectors/credentials are assigned, who has actually used it, summarized work topics/outcomes/blockers from app.list, readiness.read, activity.read, trace.read, or context.search. Do not dump raw private chats by default.
12. Verify with readiness.read, integration.check_readiness, diagnostics.read_runtime_status, and finally a real Telegram route/agent-turn or capability-use evidence before saying the Personal Agent is ready.
...
- Owner receives a people-and-access summary with assigned resources, usage/blocker signals, and no raw chat dump.
